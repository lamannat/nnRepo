...
//transformacion_exp modifica el contenido de una matriz utilizando la funcion exponencial compleja
...
void transformacion_exp(vector<vector<int>> &, int, int);
...
int main(int argc, char * const argv[])
{
    ...
    if(funcion == "exp(z)")
        imgout = imgin.transformar(transformacion_exp);
    if(funcion == "z")
        imgout = imgin;
...
opt_function(string const &arg){
    ...
    if(arg == "z" || arg == "exp(z)"){
        ...
        cerr<<"argumento de funcion no valido. Utilizar 'z' o 'exp(z)' como argumento"<<endl;
        exit(1);
    }
...
void transformacion_exp(vector<vector<int>> & matriz, int ancho, int alto){
